window.addEventListener('load', function () {
    ...
    function drawCrepeDelightMetrics(dataRows) {
        ...
        document.getElementById('totalRevenueValue').innerText = `RM${totalRevenue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
        document.getElementById('totalorderCount').innerText = totalOrders.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    ...
    function drawTopSellingItems(dataRows) {
        ...
        if (topItems.length > 0) {
            ...
            document.getElementById('mostSoldItem').innerText = topItem.name;
        } else {
            document.getElementById('mostSoldItem').innerText = 'No data';
        }
...
*/
    ...
    function drawCustomerMetrics(dataRows) {
        ...
        document.getElementById('uniqueCustomerCount').innerText = `${uniqueCustomers} unique customers`;
...
function populateOrderTable(dataRows) {
    const tableBody = document.querySelector('#orderTable tbody'); // Adjust ID if needed
    tableBody.innerHTML = ''; // Clear existing rows

    // Sort by order_time descending (most recent first)
    const sortedData = dataRows
        .filter(row => row.order_time && !isNaN(new Date(row.order_time)))
        .sort((a, b) => new Date(b.order_time) - new Date(a.order_time))
        .slice(0, 10); // ✅ Take only the top 10 recent orders

    sortedData.forEach(row => {
        const orderId = row.order_id || '-';
        const customer = row.eater_id || '-';
        const menu = row.item_name || '-';
        const orderDate = new Date(row.order_time);
        const formattedDate = orderDate.toLocaleDateString('en-MY', {
            year: 'numeric', month: 'short', day: 'numeric'
        });

        const amount = `RM${parseFloat(row.item_price || 0).toFixed(2)}`;
        const status = row.order_status?.toLowerCase() === 'pending' ? 'pending' : 'completed';
